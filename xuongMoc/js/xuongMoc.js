...
$(document).on('click', '.them2', function(){
    let img=$(this).parent().siblings(".image").children('a').children('img').attr("src");
...
$(document).on("change",'.soluong input[type="number"]', function(){
    ...
    let tong=0;
        ...
        $(this).parent().parent().parent().parent().parent().parent().siblings(".miniCart-footer").children(".miniCart-total").children('span.miniCart-price').attr("data-value",tong);
});
$(document).on('click', '.rm', function(){
    let removeItem=$(this).parent().parent();
    removeItem.remove();
    let tong= $(this).parent(".cartItem-img").parent(".miniCart-item").parent().text();
    console.log(tong)
    let t = $(this).attr("data-value");
